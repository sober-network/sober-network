...
public sealed class NewsService(AppDbContext db) : INewsService
{
        ...
        Guid userId, int page, int pageSize, CancellationToken ct = default)
    {
        ...
        var items = posts.Select(p =>
            MapToResponse(p, authors.GetValueOrDefault(p.AuthorId, "Unknown"), adminGroupIds, isSuperAdmin)).ToList();
        ...
        Guid userId, CreatePostRequest request, CancellationToken ct = default)
    {
        ...
        post.Group = group;
        return (MapToResponse(post, author?.DisplayName ?? "Unknown", [], isSuperAdmin: false), null);
    }
    ...
    private static PostResponse MapToResponse(
        Post post, string authorDisplayName, ICollection<Guid> adminGroupIds, bool isSuperAdmin) =>
        new(
            post.Id,
            post.GroupId,
            post.Group?.Name ?? string.Empty,
            post.Group?.Slug ?? string.Empty,
            post.AuthorId,
            authorDisplayName,
            post.Subject,
            post.Body,
